...
public class AdminOrderListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		OrderListService service=OrderListService.getService();//BoardService에서 싱글톤패턴으로 짯엇음
		...
		List<OrderListDTO> list=service.getList(startrow,endrow,search,txtsearch, stxtsearch1, stxtsearch2);//이건 걍 전체 가져오는거고(페이징으로)
	...
	/*	String addpoint=request.getParameter("addpoint");
		...
		f.setUrl("/adminOrderList.jsp");//list.jsp로 넘김. 근데 얜 왜 jsp인지 모르겟음. 슬래시 써야함
