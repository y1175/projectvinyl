...
public class QuestionDAO {
	...
	public int totoalQcount(Connection conn, String questionsearchtxt, String questionsearch) throws SQLException{
		...
		if(!("".equals(questionsearchtxt)) && !("".equals(questionsearch))) {
			if("title".equals(questionsearch))
				sql.append("  where      title     like  ?             ");
			else if("content".equals(questionsearch))
				sql.append("  where      content     like  ?           ");
		}
		try(PreparedStatement ps=conn.prepareStatement(sql.toString());) {
			if(!("".equals(questionsearchtxt)) && !("".equals(questionsearch))) {
				ps.setString(1, "%"+questionsearchtxt+"%");
			}
			rs=ps.executeQuery();
			if(rs.next()) {
	...
	public List<QuestionDTO> getQlist(Connection conn, int startrow, int endrow, String questionsearch, String questionsearchtxt) throws SQLException{
		// TODO Auto-generated method stub
		ResultSet rs=null;
        ...
        if(questionsearch.equals("title"))
        	sql.append("     where   title   like ?      ");
        else if(questionsearch.equals("content"))
        	sql.append("     where   content   like ?    ");
        //페이징                                       시작페이지    페이지사이즈
        sql.append("  limit       ?,     5              ");
        
		try(PreparedStatement ps=conn.prepareStatement(sql.toString())){
			if(questionsearch.equals("")) {//검색한게 없으면
