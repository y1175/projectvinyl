...
<!-- jQuery -->
<link rel="stylesheet" href="css/_adminboard.css?ver=1">
...
<form method="post" action="adminboard.do"><!--처음에 list.do햇음  -->
<div class="_admin_titlediv">
       <div class="_admin_searchbox">
        <select name="search" class="_adminsearch_searchop">
        	<option value="id">아이디</option>
			<option value="name">이름</option>
			<option value="btitle">글제목</option>
        </select> 
        <i class='fas fa-search' style="padding: 5px;"></i>
        <input type="text" id="_adminsearchtxt" name="txtsearch" placeholder="검색" >
       </div>
...
<tbody>
	...
	<td><a href="boarddelete.do?bno=${item.bno}" class="btn btn-danger">삭제</a><td>
...
<c:if test="${startblock > 1 }">
	<li><a class="page-link" href="adminboard.do?curr=${startblock-1 }&search=${search }&txtsearch=${txtsearch }">이전 블럭으로</a></li>
</c:if>

<c:if test="${currpage > 1 }">
	<li><a class="page-link" href = "adminboard.do?curr=${currpage -1 }&search=${search }&txtsearch=${txtsearch}">이전</a></li>
</c:if>

<c:forEach var = "i" begin="${startblock}" end="${endblock}" step ="1">
	<c:if test="${currpage ==i }">
		<!-- 같으면 그냥 출력 -->
		<li class="page-item active" ><a class="page-link" href="adminboard.do?curr=${i}" ><c:out value="${i}" /></a></li>
	</c:if>
	<c:if test="${currpage != i }">
		<!-- 다르면 링크 걸어주고 시작 -->
		<li><a class="page-link" href="adminboard.do?curr=${i}&search=${search}&txtsearch=${txtsearch}">${i}</a></li>
...
<c:if test="${currpage < totalpage }">
	<li><a class="page-link" href = "adminboard.do?curr=${currpage +1 }&search=${search }&txtsearch=${txtsearch}">다음</a></li>
</c:if>

<c:if test="${endblock < totalpage }">
	<li><a class="page-link" href = "adminboard.do?curr=${endblock +1 }&search=${search }&txtsearch=${txtsearch}">다음블럭으로</a></li>
</c:if>
</ul>
<div class="col"></div>
</div>
