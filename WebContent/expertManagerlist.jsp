...
<!-- jQuery -->
<link rel="stylesheet" href="css/managerlist.css?ver=2">
</head>
<body>
	...
	<form method="post" action="managerlist.do">
		...
		</select> <input type="submit" value="검색" class="searchmg">
	</form>
...
<table class="table"  >
 ...
 <%
      ...
      if (startblock > 1) {
    	  ...
    	  {
    	%>
    	 <a href="managerlist.do?curr=<%=currpage - 1%>">이전</a>
    	  <% 
    	  }else{
    	  
   %>
   <a href="managerlist.do?curr=<%=currpage - 1%>&hmcity=<%=hmcity%>">이전</a>
   <%
   ...
   %>
   <a href="managerlist.do?curr=<%=i%>"><%=i%></a>
   <% 
      }else{
   %>
   <a href="managerlist.do?curr=<%=i%>&hmcity=<%=hmcity%>"><%=i%></a>
   <%
      }
   %>
   <%
   ...
   <%
      if (endblock < totalpage) {
    	  if(hmcity==null||hmcity.equals("")){
    		  %>
    		  <a href="managerlist.do?curr=<%=currpage + 1%>">다음</a>
    		  
    		  <% 
    	  }else{
   %>
   <a href="managerlist.do?curr=<%=currpage + 1%>&hmcity=<%=hmcity%>">다음</a>
   <%
    	  }
