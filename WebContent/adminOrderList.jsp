...
<div class="container">
     <header>
         <jsp:include page="/memberadminheader.jsp"></jsp:include>
     </header>
     <div class="all">
<img src="http://localhost:9080/projectvinyl/order/delivery.png" alt="delivery"
 class="delivermage"><h3>주문배송관리</h3>
...
<select name="search" class="searchop"><!--search txtsearch받아야댐 ListAction에서  -->
	<option value="orderno">주문번호 </option>
	<option value="memno">회원번호 </option>
</select>

<input type="text" name="txtsearch" 
class="txtsearch" placeholder="주문 배송을 검색해보세요">
</div>
<br>
<div id="line"></div>

<label for="cost">가격</label>
<input type="text" name="stxtsearch1" class="stxtsearch1" placeholder="원">~
<input type="text" name="stxtsearch2" class="stxtsearch2" placeholder="원">
<input type="image" 
src="http://localhost:9080/projectvinyl/order/search.png" alt="search"
id="search">
<!-- <input  type="submit" value="상세검색" id="submit" alt="submit"> -->
...
<c:set var="temp" value="취소승인"></c:set>
	...
	<button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#exampleModal"
	 data-whatever="${item.orderno }" id="orderbutton">${temp }</button></td>
	 <!-- btn btn-primary 클래스 이거 엿음 -->
<%-- 	<td><a href="update.do?orderno=${item.orderno}" target="_blank" >${temp}</a> --%>
	
	
	<td>${item.cost }원</td>
	<td><a class="btn btn-outline-secondary" 
	href="orderdelete.do?orderno=${item.orderno}" onclick="delconfirm();">삭제</a></td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <li>
            ...
            <input type="text" class="form-control" id="recipient-name" name="what" readonly="readonly">
            </li></ul>
          </div>
      </div>
...
<input type="image" 
src="http://localhost:9080/projectvinyl/order/check.png" alt="check"
id="check">
...
<c:forEach var="i" begin="${starblock }+1" end="${endblock }" step="1">
	...
	<button type="button" class="btn btn-default" 
	 onclick="location.href='orderlist.do?curr=${i }&search=${search}&txtsearch=${txtsearch }'">${i }</button>
	<%-- <c:out value="${i }"/> --%>
	...
	<c:if test="${currpage!=i }">
	 ...
	 onclick="location.href='orderlist.do?curr=${i }&search=${search}&txtsearch=${txtsearch }'">${i }</button>
	 <!-- 이전에 햇던거 다시쓸수도있음 -->
	<%-- <a href="orderlist.do?curr=${i }&search=${search}&txtsearch=${txtsearch }">${i }</a> --%>
	</c:if>
</c:forEach><br>
</div>
</div>
